...
def write2disk(file_path, data_path, file_data):
    ...
    if not isExits:
        ...
        for i in ll:
           k= os.path.join(file_path, i)
           os.remove(os.path.join(file_path,i))
    file_data.save(data_path)
    file_path = './ImagesDB/input'
    # print(os.getcwd())
    for i in os.listdir(file_path):
        i=os.path.join(os.path.join(os.getcwd(),'ImagesDB/input'),i)
        key = i[-4:]
        key=str('input'+key)
        key=os.path.join(os.path.join(os.getcwd(),'ImagesDB/input'),key)
        print(key)
        os.rename(i,key)
    print("完成")
# get img_shape
...
def build_parser():
    ...
    parser.add_argument('--8', type=str,
                        dest='checkpoint_dir',
                        help='dir or .ckpt file to load 8 from',
